    ...
    <div class="product-detail-container">
        <div class="product-grid">
            ...
            <div class="viewer-wrapper">
                ...
                {{-- GALLERY PANORAMA PRODUK (Below 3D Model) --}}
                @if($panorama_produks && $panorama_produks->count() > 0)
                    <div class="gallery-section" style="margin-top: 24px;">
                        <div class="section-header">
                            <div class="header-line"></div>
                            <h3 class="section-title">🌄 Galeri Panorama {{ $name }}</h3>
                        </div>
                        <div class="carousel-container">
                            <div class="carousel-wrapper">
                                <div class="carousel-track" id="panorama-track">
                                    @foreach($panorama_produks as $index => $panorama)
                                        <div class="carousel-item"
                                            onclick="openPanoramaViewer({{ $index }}, {{ $panorama_produks->toJson() }})">
                                            <img src="{{ Storage::url($panorama->image) }}" alt="Panorama Produk"
                                                loading="lazy">
                                        </div>
                                    @endforeach
                                </div>
                            </div>
                            @if($panorama_produks->count() > 3)
            ...
            <div class="product-panel">
                ...
                <div class="content-area">
                    ...
                    @if($gambar_produks && $gambar_produks->count() > 0)
                        <div class="gallery-section">
                            <div class="section-header">
                                <div class="header-line"></div>
                                <h3 class="section-title">📸 Galeri Gambar {{ $name }}</h3>
                            </div>
                            <div class="carousel-container">
                                <div class="carousel-wrapper">
                                    <div class="carousel-track" id="gambar-track">
                                        @foreach($gambar_produks as $gambar)
                                            <div class="carousel-item"
                                                onclick="window.open('{{ Storage::url($gambar->image) }}', '_blank')">
                                                <img src="{{ Storage::url($gambar->image) }}" alt="Gambar Produk"
                                                    loading="lazy">
                                            </div>
                                        @endforeach
                                    </div>
                                </div>
                                @if($gambar_produks->count() > 2)
